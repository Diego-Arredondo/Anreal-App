...
def send_activate(sock):
    sock.sendall("Buenos dias".encode())
...
def send_data(sock):
    while (init):
        # Solicitar al usuario que ingrese datos para enviar al servidor
        msg = input("Ingrese mensaje (q = parar) (Threshold (+0), RepMargin (0), repValue (-2)): ")
...
def quit_threads():
    ...
    while (init):
        # Solicitar al usuario que ingrese datos para enviar al servidor
        msg = input("Ingrese mensaje q = parar: ")
        if msg == "q":
            running = False
            break
...
def my_function(i):
    ...
    # time vector
    time = [i*0.06 for i in range(GRAPH_LENGTH)]
...
if __name__ == "__main__":
    ...
    # Iniciar un hilo para la interfaz grafica
    # threading.Thread(target=gui.start_gui).start()
    gui.start_gui()
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        # Iniciar un hilo para enviar datos al servidor
        # send_thread = threading.Thread(target=send_data, args=(sock,))
        # send_thread.start()

        #Iniciar un hilo para cerrar los hilos
        quit_thread = threading.Thread(target=quit_threads)
        quit_thread.start()
        ...
        print("Hilos terminaron")

        gui.current_user.values = data_user

        gui.current_user.flags = flags_user

        gui.current_user.maxs = maximos

        gui.current_user.save()
      
        plot_histogram()

        #plot_data()

        gui.current_user.save_excel()
